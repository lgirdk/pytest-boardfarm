...
### LLM-assisted test generation prompt — `pytest_boardfarm3/scripts/docs/boardfarm3_pytest_generator.md` *(WIP)*

- **What it is used for *(WIP)*:** a self-contained system prompt that drives an LLM (Claude Project, Custom GPT, raw API `system` field, or plain chat UI) to convert a testcase requirements / specification document into runnable `pytest` test files for the public `boardfarm3` framework. The prompt bundles every rule the model needs — hard constraints, the authoritative Device API surface, output format, style rules, and a pre-send self-check — so it can be pasted in one shot with no companion files and no web access. Its main job is to **prevent common LLM hallucinations** when generating boardfarm3 tests (invented method names, PEP-8 normalisation of acronym methods, fabricated attribute access on custom return types, etc.).
- **How the .md is created *(WIP)*:**
  - Narrative sections (How to use, Hard constraints, Output format, Style rules, Pre-output self-check) are maintained by hand.
  - The **Device API surface** block delimited by `<!-- BEGIN AUTO-GENERATED API SURFACE -->` / `<!-- END AUTO-GENERATED API SURFACE -->` is produced by introspecting every Template ABC under `boardfarm3.templates.*` (and any other templates could be provided as a cmd line arg during creation) and emitting their public properties, method signatures, and return-type annotations. Regenerating this block rewrites only the region between the markers; everything outside is left untouched. The method names, argument lists, and return annotations in that block are treated as the **contract** the LLM must follow byte-for-byte.
  - This `.md` can be (re)generated by running `pytest_boardfarm3/scripts/test_context_generator.py`, which performs the Template ABC introspection above and rewrites the auto-generated API surface block in place.
- **Environment specifics *(WIP)*:** the actual environment pre-requisite creation for pytest is not handled — is **still to be documented**.
